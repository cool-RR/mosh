...
#if WCHAR_MAX <= 0xFFFF
      /* Handle UTF-16 surrogate pairs - don't try to predict these,
         just let the server state take over */
      const uint32_t chcheck = ch;
      if ( chcheck >= 0xD800 && chcheck <= 0xDBFF ) {
        ...
        /* Low surrogate - skip prediction for supplementary plane chars */
